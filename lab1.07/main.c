...
enum Print_edition { BOOK = 1, MAGAZINE, NEWSPAPER };
...
int main() {
  ...
  printf("the perimeter of a triangle with sides %d, %d, %d is %d\n",
         triangle.a, triangle.b, triangle.c, get_perimeter(&triangle));

  union Condition condition;
  scanf("%x", &condition.number);
  printf("%d\n%d\n%d\n%d\n", condition.byte.included,
         condition.byte.sd_card_active,
         condition.byte.compact_flash_card_active,
         condition.byte.MemoyStick_card_active);
